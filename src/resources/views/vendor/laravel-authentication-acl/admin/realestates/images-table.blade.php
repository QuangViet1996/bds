...
if (!empty($realestate)) {
    $arr = $realestate->real_estate_images;
    $arr_images = json_decode($realestate->real_estate_images);
}
?>

@if($realestate)
<div class="img-thumb">
    <img src="{!! url($data['configs']['urlpath'].'/'.$realestate->real_estate_image) !!}">
</div>


<div class="showfile">
    <i class=" fa fa-file-excel-o" aria-hidden="true"></i>
    <span class="filename">{!! $realestate->real_estate_image !!}</span>
...
@endif
{{ Form::hidden('is_file', !empty(@$realestate->real_estate_image)?1:0, ['class' => 'is_file']) }}
