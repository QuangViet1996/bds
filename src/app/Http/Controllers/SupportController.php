...
class SupportController extends Controller {
    ...
    public function getSupports(Request $request) {
        ...
        $obj_payroll_support_cat = new PayrollSupportCat();

        $input = $request->all();
        $supports = $obj_payroll_support->getPayrollSupportList($input);

        $data = array_merge($this->data, array(
            'supports' => $supports,
            'payroll_support_categories' => $obj_payroll_support_cat->getPayrollCat(),
            'request' => $request
        ));

        return View::make('laravel-authentication-acl::admin.support.supports')->with(['data' => $data]);
    }
